...
## 스케줄
Vercel Hobby Cron은 일 단위 작업을 시간 단위 정밀도로 실행할 수 있으므로 UI/문서에서 특정 23:30 정각을 보장하지 않습니다. Primary reminder는 23시대(KST)에 실행하고, 자정 이후 같은 Study Day에 backup reminder를 한 번 더 실행합니다. 기기별 delivery 멱등 처리로 이미 성공한 기기에는 중복 발송하지 않고 실패/미발송 기기만 재시도합니다.
