...
export async function POST(request: Request) {
  try {
    ...
    const { data: membership } = await admin.from(DB.studyMembers).select("study_id").eq("study_id", studyId).eq("user_id", user.id).maybeSingle();
    ...
    await admin.from(DB.githubInstallations).upsert({ installation_id: installationId, user_id: user.id });
    for (const repo of result.repositories) {
      await admin.from(DB.repositoryConnections).upsert({ user_id: user.id, study_id: studyId, installation_id: installationId, github_repository_id: repo.id, full_name: repo.full_name, active: true }, { onConflict: "github_repository_id" });
    }
